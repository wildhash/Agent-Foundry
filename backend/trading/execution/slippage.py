...
class SlippageModel:
    ...
    def __init__(self, impact_coefficient: float = 0.1, permanent_impact_ratio: float = 0.5):
        """
    ...
    def estimate(
        self, order_size: float, daily_volume: float, volatility: float, spread_bps: float, is_buy: bool = True
    ) -> SlippageEstimate:
        """
        ...
        # impact = η * σ * √(Q/V)
        market_impact = self.impact_coefficient * volatility * np.sqrt(participation) * 10000  # Convert to bps
        ...
        return SlippageEstimate(
            spread_cost_bps=spread_cost_bps, market_impact_bps=market_impact, total_cost_bps=total_cost, confidence=confidence
        )
    ...
    def estimate_execution_price(
        ...
        """
        estimate = self.estimate(order_size, daily_volume, volatility, spread_bps, is_buy)
    ...
    def record_execution(self, predicted_bps: float, actual_bps: float):
        """
