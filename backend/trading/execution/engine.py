...
class ExecutionEngine:
    ...
    def __init__(self, default_algorithm: str = "twap", max_order_value: float = 100000.0, min_order_value: float = 100.0):
    ...
    def create_order(
        ...
        if trade_value > self.max_order_value:
            trade_quantity = self.max_order_value / current_price * np.sign(trade_quantity)
        ...
        return TradeOrder(
            ...
            metadata={"target_position": target.percent_of_nav, "regime": regime.value, "signal_strength": target.raw_signal},
        )

    def _select_order_type(self, regime: MarketRegime, order_value: float) -> OrderType:
        """
    ...
    async def submit(
        ...
        # Create order state
        state = OrderState(order=order, status=OrderStatus.SUBMITTED, submitted_at=datetime.now(), slippage_estimate=estimate)
        ...
        if order.order_type in [OrderType.TWAP, OrderType.VWAP]:
            ...
            state.child_orders = algo.generate_schedule(total_quantity=order.quantity, limit_price=order.limit_price)
    ...
    def record_fill(self, order_id: str, fill_quantity: float, fill_price: float, timestamp: Optional[datetime] = None):
        """
        ...
        if state.filled_quantity > 0:
            state.avg_fill_price = (old_filled * state.avg_fill_price + fill_quantity * fill_price) / state.filled_quantity

        # Record fill
        state.fills.append({"quantity": fill_quantity, "price": fill_price, "timestamp": timestamp or datetime.now()})
    ...
    def _on_order_complete(self, state: OrderState):
        ...
        if state.slippage_estimate:
            ...
            self.slippage_model.record_execution(predicted, state.actual_slippage_bps)

        # Move to completed
        self.completed_orders.append(state)
        del self.orders[order.client_order_id]

        logger.info(f"Order completed: {order.client_order_id} " f"avg_price={state.avg_fill_price:.2f}")
    ...
    def get_execution_style(self, regime: MarketRegime, urgency: float = 0.5) -> Dict:
        """
    ...
    def get_statistics(self) -> Dict:
        ...
        slippages = [o.actual_slippage_bps for o in filled_orders if o.actual_slippage_bps > 0]
