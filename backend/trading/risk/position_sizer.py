...
class VolatilityTargetedSizer:
    ...
    def calculate_realized_vol(self, returns: np.ndarray, annualize: bool = True, use_ewma: bool = True) -> float:
        """
    ...
    def size_position(
        self, signal: AlphaSignal, asset_vol: float, nav: float, risk_budget_pct: float = 1.0, current_price: float = 1.0
    ) -> PositionSize:
        """
        ...
        asset_vol = np.clip(asset_vol, self.config.vol_floor, self.config.vol_ceiling)
        ...
        position_pct = np.clip(raw_position_pct, -self.config.max_leverage, self.config.max_leverage)

        # Apply single position limit
        position_pct = np.clip(position_pct, -self.config.max_position_pct, self.config.max_position_pct)
    ...
    def kelly_size(self, win_rate: float, win_loss_ratio: float, max_pct: float = 0.25) -> float:
        """
