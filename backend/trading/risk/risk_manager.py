...
class RiskManager:
    ...
    def update_nav(self, nav: float):
        ...
        if self.current_drawdown >= self.limits.max_drawdown_pct:
            self._activate_kill_switch(f"Max drawdown exceeded: {self.current_drawdown:.2%}")
    ...
    def update_pnl(self, pnl: float):
        ...
        if self.daily_loss_pct >= self.limits.max_daily_loss_pct:
            self._activate_kill_switch(f"Daily loss limit exceeded: {self.daily_loss_pct:.2%}")
    ...
    def check_limits(
        ...
        if self.daily_loss_pct >= self.limits.max_daily_loss_pct:
            violations.append(
                f"{RiskViolationType.DAILY_LOSS.value}: " f"{self.daily_loss_pct:.2%} >= {self.limits.max_daily_loss_pct:.2%}"
            )
        ...
        if sector:
            ...
            if abs(new_sector) > self.limits.max_sector_exposure:
                violations.append(
                    f"{RiskViolationType.SECTOR_EXPOSURE.value}: " f"{new_sector:.2%} > {self.limits.max_sector_exposure:.2%}"
                )

        # 6. Leverage limit
        new_exposure = self.state.total_exposure + abs(proposed_position.percent_of_nav)
        if new_exposure > self.limits.max_leverage:
            violations.append(f"{RiskViolationType.LEVERAGE.value}: " f"{new_exposure:.2f}x > {self.limits.max_leverage:.2f}x")
        ...
        if violations:
            return RiskCheckResult(approved=False, violations=violations, adjusted_position=None, risk_score=risk_score)

        return RiskCheckResult(approved=True, violations=[], adjusted_position=proposed_position, risk_score=risk_score)
    ...
    def scale_for_risk(self, position: PositionSize, urgency: float = 0.5) -> PositionSize:
        """
    ...
    def set_stop_loss(
        self, symbol: str, entry_price: float, stop_pct: float = 0.02, vol_scaled: bool = True, current_vol: float = 0.15
    ):
        """
        ...
        logger.info(f"Set stop for {symbol}: entry={entry_price:.2f}, " f"stop={stop_price:.2f} ({adjusted_stop_pct:.2%})")
    ...
    def check_strategy_health(self, strategy: str, min_sharpe: float = 0.5, min_trades: int = 10) -> bool:
        """
