...
class FeatureEngine:
    ...
    def __init__(self, max_history: int = 1000, default_lookback: int = 252):
    ...
    def update(self, data: MarketData):
        ...
        if symbol in self._data:
            self._data[symbol] = pd.concat([self._data[symbol], new_row], ignore_index=True)
    ...
    def get_features(self, symbol: str, timestamp: Optional[datetime] = None) -> FeatureSet:
        """
        Get all features for a symbol.
        ...
        if symbol not in self._data:
            return FeatureSet(symbol=symbol, timestamp=timestamp or datetime.now(), features={})
        ...
        return FeatureSet(
            symbol=symbol, timestamp=timestamp or datetime.now(), features={k: v[-1] for k, v in features_dict.items()}
        )

    def _calculate_all_features(self, df: pd.DataFrame) -> Dict[str, np.ndarray]:
        """
        ...
        if "bid_size" in df.columns and "ask_size" in df.columns:
            features["order_book_imbalance"] = VolumeFeatures.order_book_imbalance(
                df["bid_size"].values, df["ask_size"].values
            )
        ...
        # Derived features
        features["vol_of_vol"] = PriceFeatures.realized_volatility(np.diff(vol, prepend=vol[0]), 20, annualize=False)
    ...
    def _normalize_features(self, features: Dict[str, np.ndarray], lookback: int = 100) -> Dict[str, np.ndarray]:
        """
