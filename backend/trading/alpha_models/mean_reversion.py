...
class MeanReversionAlphaModel(BaseAlphaModel):
    ...
    def __init__(
        ...
        super().__init__(name=name, lookback_periods=max(lookback, hurst_lookback) + 10, min_data_points=lookback)
    ...
    def get_required_features(self) -> list:
        ...
        return ["zscore", "hurst_exponent", "bollinger_position", "realized_vol"]
    ...
    def generate_signal(self, ohlcv: pd.DataFrame, features: Optional[FeatureSet] = None) -> AlphaSignal:
        ...
        if abs(zscore) < self.entry_threshold:
            signal = AlphaSignal(
                ...
                components={"hurst": hurst, "zscore": zscore, "threshold": self.entry_threshold, "bb_position": bb_position},
                model_name=self.name,
                timestamp=datetime.now(),
            )
            self._store_signal(signal)
        ...
        signal = AlphaSignal(
            ...
            components={"hurst": hurst, "zscore": zscore, "bb_position": bb_position, "entry_threshold": self.entry_threshold},
            model_name=self.name,
            timestamp=datetime.now(),
        )
    ...
    def _calculate_bollinger_position(self, prices: np.ndarray, num_std: float = 2.0) -> float:
        """
