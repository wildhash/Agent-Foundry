...
class MomentumAlphaModel(BaseAlphaModel):
    ...
    def __init__(
        ...
        super().__init__(name=name, lookback_periods=max(lookbacks) + adx_period + 10, min_data_points=max(lookbacks))
    ...
    def get_required_features(self) -> list:
        ...
        return ["log_returns", "realized_vol", "adx", "trend_direction"]
    ...
    def generate_signal(self, ohlcv: pd.DataFrame, features: Optional[FeatureSet] = None) -> AlphaSignal:
        ...
        adx = self._calculate_adx(ohlcv["high"].values, ohlcv["low"].values, ohlcv["close"].values, self.adx_period)
