...
class VolatilityBreakoutModel(BaseAlphaModel):
    ...
    def __init__(
        ...
        super().__init__(
            name=name,
            lookback_periods=max(atr_period, lookback) * 2,
            min_data_points=max(atr_period, lookback)
        )
    ...
    def generate_signal(
        ...
        if atr_ratio < self.vol_expansion_threshold:
            signal = AlphaSignal(
                ...
                components={
                    "atr": atr,
                    "atr_ratio": atr_ratio,
                    "threshold": self.vol_expansion_threshold
                },
        ...
        confirmations = sum([
            atr_ratio >= self.vol_expansion_threshold,
            abs(breakout_strength) > 0.5,
            volume_confirmed
        ])
