...
class AlphaEnsemble:
    ...
    def __init__(
        ...
        self.regime_weights: Dict[MarketRegime, Dict[str, float]] = {
            MarketRegime.TRENDING_UP: {
                "momentum": 0.6, "mean_reversion": 0.1, "volatility_breakout": 0.3
            },
            MarketRegime.TRENDING_DOWN: {
                "momentum": 0.6, "mean_reversion": 0.1, "volatility_breakout": 0.3
            },
            MarketRegime.MEAN_REVERTING: {
                "momentum": 0.1, "mean_reversion": 0.7, "volatility_breakout": 0.2
            },
            MarketRegime.HIGH_VOLATILITY: {
                "momentum": 0.3, "mean_reversion": 0.2, "volatility_breakout": 0.5
            },
            MarketRegime.LOW_VOLATILITY: {
                "momentum": 0.4, "mean_reversion": 0.5, "volatility_breakout": 0.1
            },
            MarketRegime.CRISIS: {
                "momentum": 0.0, "mean_reversion": 0.0, "volatility_breakout": 0.0
            },
            MarketRegime.NORMAL: {
                "momentum": 0.4, "mean_reversion": 0.4, "volatility_breakout": 0.2
            },
        }

        # Thompson Sampling state (alpha, beta for each model)
        self.ts_params: Dict[str, Dict[str, float]] = {
            name: {"alpha": 1.0, "beta": 1.0} for name in models
        }
    ...
    def update_thompson_params(
        ...
        for name in self.ts_params:
            self.ts_params[name]["alpha"] = max(
                1.0, self.ts_params[name]["alpha"] * decay
            )
            self.ts_params[name]["beta"] = max(
                1.0, self.ts_params[name]["beta"] * decay
            )
        ...
        for name, pnls in self.model_performance.items():
            if pnls:
                ...
                stats[name] = {
                    ...
                    "ts_mean": self.ts_params[name]["alpha"] / (
                        self.ts_params[name]["alpha"] + self.ts_params[name]["beta"]
                    )
                }
            else:
                stats[name] = {
