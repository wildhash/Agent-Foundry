...
def test_pipeline_creation(test_db):
    ...
    pipeline = Pipeline(task_description="Test task", status="initialized")
...
def test_pipeline_with_agent_executions(test_db):
    ...
    # Create pipeline
    pipeline = Pipeline(task_description="Build API", status="running")
    test_db.add(pipeline)
    test_db.flush()
...
def test_pipeline_status_enum(test_db):
    ...
    for status in valid_statuses:
        pipeline = Pipeline(task_description=f"Test {status}", status=status)
        test_db.add(pipeline)
